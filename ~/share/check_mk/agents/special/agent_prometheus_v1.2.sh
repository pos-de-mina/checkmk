...
function check_prometheus_share_mounted () {
  local promql='share_mounted{hostname="'$PROMETHEUS_HOST'"}'
  local jquery='.data.result[0].value[1]'

  local v=$(get_prometheus_metric "${promql}" "${jquery}")
  if [[ $v -eq 1 ]]; then
    printf "<<<local>>>\n0 SharedMounted - Shared Mounted\n" 
  else
    printf "<<<local>>>\n2 SharedMounted - Shared Not Mounted\n"
  fi
}
...
if [ "$PROMETHEUS_HOST" = "" ]; then
  printf 'Host %s not monitored in Prometheus(!!)\n\n' $PROMETHEUS_HOST >&2
  exit 2
fi
...
check_prometheus_share_mounted
