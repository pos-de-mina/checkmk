...
try {
    ...
    Get-ChildItem IIS:\AppPools -Exclude '*.NET*' | ForEach-Object {
        ...
        $cmk_perfdata = ''
        $i = 0
        Get-Counter -Counter ("\HTTP Service Request Queues($IISAppPool)\*", "\APP_POOL_WAS($IISAppPool)\*", "\WAS_W3WP(*_$IISAppPool)\*", "\W3SVC_W3WP(*_$IISAppPool)\*") -ErrorAction SilentlyContinue | 
